...
def rechercher_3_derniers_albums(chat_id, nom_user, requete):
    msg = bot.send_message(chat_id, f"🔎 *Recherche des 3 derniers albums pour {nom_user}...*", parse_mode="Markdown")
    ...
    try:
        with YoutubeDL(options_recherche) as ydl:
            ...
            for idx, entry in enumerate(entries[:3], 1):
                ...
                nom_clean = titre_album.replace('Full Album', '').replace('Album', '').strip()[:22]
                markup.add(InlineKeyboardButton(
                    f"💿 Album #{idx} : {nom_clean}", 
                    callback_data=f"dlalbum_{entry.get('id', titre_album)[:30]}"
                ))
...
def callback_telecharger_album_selectionne(call):
    ...
    bot.answer_callback_query(call.id, "Téléchargement de l'album...")
    msg = bot.send_message(chat_id, f"💿 *Téléchargement des pistes pour {nom_user}...*", parse_mode="Markdown")
    
    options_dl = {
        'format': 'bestaudio[ext=m4a]/bestaudio/best',
        'default_search': f'ytsearch6:{target_album}',
        'outtmpl': '%(id)s.%(ext)s',
        'quiet': True,
        'noplaylist': True
    }
    ...
    try:
        with YoutubeDL(options_dl) as ydl:
            info = ydl.extract_info(target_album, download=True)
            entries = info.get('entries', []) if info else []
        ...
        bot.delete_message(chat_id, msg.message_id)
        bot.send_message(chat_id, f"✅ Album envoyé avec succès pour **{nom_user}** !", parse_mode="Markdown")
        
    except Exception as e:
        bot.send_message(chat_id, f"⚠️ Erreur lors du téléchargement : {str(e)}")
